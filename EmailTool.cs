...
public static class EmailTool
{
    ...
    public static async Task<string> ListLastXEmails(ImapClient client, int count = 10, string? folderName = null)
    {
        IMailFolder folder = await GetFolder(client, folderName);

        await folder.OpenAsync(FolderAccess.ReadOnly);

        var uids = await folder.SearchAsync(SearchOptions.All, SearchQuery.All);
        Console.WriteLine($"Found {uids.Count} emails in the folder '{folder.Name}'.");
        var summaries = await folder.FetchAsync(uids.UniqueIds.Reverse().Take(count).ToList(), MessageSummaryItems.Envelope);

        return string.Join("\n", summaries.Select(s => $"{s.UniqueId}: {s.Envelope.MessageId}: {s.Envelope.Subject}"));
    }

    private static async Task<IMailFolder> GetFolder(ImapClient client, string? folderName)
    {
        ...
        {
            throw new InvalidOperationException($"Error: Folder '{folderName ?? "Inbox"}' does not exist.");
        }
    ...
    public static async Task<string> GetEmailHeaders(ImapClient client, uint uid, string? folderName = null)
    {
        IMailFolder folder = await GetFolder(client, folderName);

        await folder.OpenAsync(FolderAccess.ReadOnly);

        var headers = await folder.GetHeadersAsync(new UniqueId(uid));
    ...
    {
        IMailFolder folder = await GetFolder(client, args.SourceFolder);
    ...
    public static async Task<string> GetEmailBody(ImapClient client, uint uid, string? folderName = null)
    {
        IMailFolder folder = await GetFolder(client, folderName);
    ...
    public static async Task<string> SearchEmailsByBody(ImapClient client, string keyword, string? folderName = null)
    {
        IMailFolder folder = await GetFolder(client, folderName);

        await folder.OpenAsync(FolderAccess.ReadOnly);
        var uids = await folder.SearchAsync(SearchOptions.All, SearchQuery.BodyContains(keyword));
        if (uids.Count == 0)
        {
    ...
    public static async Task<string> SearchEmailsBySubjectAndAuthor(ImapClient client, string subject, string? author, string? folderName = null)
    {
        IMailFolder folder = await GetFolder(client, folderName);
        ...
        var uids = await folder.SearchAsync(SearchOptions.All, query);
